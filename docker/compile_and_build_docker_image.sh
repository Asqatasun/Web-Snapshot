...
usage: $0 -s <directory> -d <directory> [OPTIONS]
  ...
  -s | --source-dir      <directory>   MANDATORY Absolute path to Web-Snapshot sources directory 
  -d | --docker-dir      <directory>   MANDATORY Path to directory containing the Dockerfile. Path must be relative to SOURCE_DIR
  -p | --port            <port>        Default value: 8895
  -n | --container-name  <name>        Default value: web.snapshot
  -i | --image-name      <name>        Default value: asqatasun/web-snapshot
...
declare CONTAINER_EXPOSED_PORT="8895"
declare CONTAINER_NAME="web.snapshot"
declare IMAGE_NAME="asqatasun/web-snapshot"
...
TGZ_BASENAME="websnapshot-webapp/target/websnapshot"
TGZ_EXT=".tar.gz"
ADD_IP=''
URL="http://localhost:${CONTAINER_EXPOSED_PORT}/websnapshot/"
if ${ONLY_LOCALHOST} ; then  
    ADD_IP="127.0.0.1:";
    URL="http://127.0.0.1:${CONTAINER_EXPOSED_PORT}/websnapshot/"
fi
